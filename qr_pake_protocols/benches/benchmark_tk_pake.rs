...
async fn main() -> io::Result<()> {
    ...
    // Create worksheet and write headers before the loop
    {
        ...
        for protocol_variant in AvailableVariants::iter() {
            ...
            println!(
                    "\x1b[93m Benchmarking {:?} variant of TK-PAKE for {} iterations without Client Authentication. \x1b[0m",
            ...
            let (
                ...
                communication_cost_server,
            ) = execute_tk_pake(registered_clients, false).await?;
    ...
    let mut filename = String::from("tk_pake_data.xlsx");
    while Path::new(&filename).exists() {
        file_index += 1;
        filename = format!("tk_pake_data_{}.xlsx", file_index);
    }
