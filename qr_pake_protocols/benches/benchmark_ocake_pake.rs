use qr_pake_protocol_executors::bench_api::ocake_pake::{
    execute_ocake_pake, perform_ocake_pake_client_registration,
};
...
fn write_results(
    ...
    worksheet.write_string(row as u32, 0, kem_choice).unwrap();
    worksheet
        .write_string(row as u32, 1, protocol_variant)
        .unwrap();
    worksheet
        .write_string(
            row as u32,
...
async fn register_100_clients(
...
) -> io::Result<Vec<(OcakeClient, OcakeServer)>> {
    let mut registered_clients: Vec<(OcakeClient, OcakeServer)> = Vec::new();
    ...
    for _ in 0..ITERATION_COUNT {
        ...
        let (tk_client_instance, tk_server_instance) = perform_ocake_pake_client_registration(
            &client_id,
            &client_password,
            protocol_variant,
            kem_choice,
...
async fn main() -> io::Result<()> {
    ...
    // Write headers
    worksheet.write_string(0, 0, "KEM Choice").unwrap();
    worksheet.write_string(0, 1, "Protocol Variant").unwrap();
    worksheet
    ...
    for kem in KemChoice::iter() {
        for protocol_variant in AvailableVariants::iter() {
            let registered_clients: Vec<(OcakeClient, OcakeServer)> =
                register_100_clients(protocol_variant, kem).await.unwrap();

            println!(
                "\x1b[93m Benchmarking {:?} variant of OCAKE-PAKE for {} iterations with {:?}. \x1b[0m",
                protocol_variant, ITERATION_COUNT, kem
            );
            ...
            let (
                ...
                communication_cost_server,
            ) = execute_ocake_pake(registered_clients).await?;
    ...
    let mut filename = String::from("ocake_pake_data.xlsx");
    while Path::new(&filename).exists() {
        file_index += 1;
        filename = format!("ocake_pake_data_{}.xlsx", file_index);
    }
