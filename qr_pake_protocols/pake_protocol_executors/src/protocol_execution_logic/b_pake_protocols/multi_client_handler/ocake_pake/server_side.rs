...
use qr_pake_protocols::{to_human_readable, AvailableVariants, KemChoice, OcakeServer};
...
impl ServerHandler for OcakeServer {
    type ServerType = OcakeServer;
...
/// Handles the registration process for a client over a TCP stream for `Modified OCAKE-PAKE`
///
/// # Parameters:
...
async fn registration_handler(
    ...
    storage: Arc<dyn Storage<OcakeServer> + Send + Sync>,
) {
    ...
    let (client_id, client_password, protocol_variant, kem_choice): (
        Vec<u8>,
        Vec<u8>,
        AvailableVariants,
        KemChoice,
    ) = from_slice(&buffer).unwrap();
    println!("INFO: Client chose {:?}-KEM", kem_choice);
    ...
    let client_already_registered =
        storage.client_exists(&client_id, protocol_variant, Some(kem_choice));

    match client_already_registered {
        Ok(true) => {
            eprintln!("WARNING: Client ID {} with {:?} {:?} variant is already registered. Skipping registration.", to_human_readable(&client_id), kem_choice, protocol_variant);
        ...
        Ok(false) => {
            ...
            ocake_server.accept_registration(client_id.clone(), client_password, kem_choice);

            // Store the server instance in the storage
            storage
                .insert_server_instance(
                    client_id.clone(),
                    protocol_variant,
                    Some(kem_choice),
                    &ocake_server,
...
/// Handles the login process for a client over a TCP stream for `Modified OCAKE-PAKE`
///
/// # Parameters:
/// - `socket`: `TcpStream,` - The TCP stream representing the client connection.
/// - `storage`: `Arc<dyn Storage<Self::ServerType> + Send + Sync>,` - Shared storage backend for managing server state.
...
async fn login_handler(
    ...
    // Receive the client_id the protocol_variant and the KEM for login
    let (client_id, protocol_variant, kem_choice): (Vec<u8>, AvailableVariants, KemChoice) =
        from_slice(&buffer).unwrap();
    println!(
    ...
    let exists: bool = storage
        .client_exists(&client_id, protocol_variant, Some(kem_choice))
        .expect(
            format!(
                "ERROR: Existence checking from storage for client {} failed",
                to_human_readable(&client_id)
    ...
    if !exists {
        ...
        let _ = socket.write_all(b"ERROR: The Client ID with the chosen KEM variant is not registered. So, the client cannot login.").await;
        return;
    }

    // Check once if client is already blocked or not
    if storage
        .is_client_blocked(&client_id, protocol_variant, Some(kem_choice))
        .unwrap_or(false)
    {
        eprintln!(
        ...
        let _ = socket
            .write_all(b"ERROR: The Client ID with the chosen KEM variant is already blocked due to multiple consecutive incorrect login attempts.")
            .await;
        return;
    }

    // Get the server state from the storage which was stored after client registration
    let mut ocake_server: OcakeServer = match storage
        .get_stored_server_instance(client_id.clone(), protocol_variant, Some(kem_choice))
    ...
    let client_message_size = u32::from_le_bytes(server_buffer_size) as usize;
    let mut buffer = vec![0u8; client_message_size];
    socket.read_exact(&mut buffer).await.unwrap();

    // Deserialize the data into a tuple (c, nonce_c)
    ...
    let read_msg_result = timeout(Duration::from_secs(resp_timeout), socket.read_exact(&mut buffer)).await;
    ...
    // Check if client sent explicit error message instead of tag_two
    ...
    match ocake_server.server_finish(&tag_two) {
        Ok(_shared_session_key) => {
            ...
            let current_count: usize = storage
                .get_incorrect_login_attempts_count(&client_id, protocol_variant, Some(kem_choice))
                .unwrap_or(0);

            if current_count > 0 {
                // Reset failed login attempts in case of correct password
                let _ = storage.reset_incorrect_login_attempts(
                    &client_id,
                    protocol_variant,
                    Some(kem_choice),
                );
...
async fn handle_incorrect_login_attempt(
    ...
    let count: usize = storage
        .record_incorrect_login_attempt(
            client_id,
            protocol_variant,
            Some(kem_choice),
            login_window,
    ...
    if count == login_threshold {
        let _ = storage.block_client(client_id, protocol_variant, Some(kem_choice));
        eprintln!(
        ...
        let msg: String = format!(
            "ERROR: The Client ID with the chosen KEM variant is blocked due to {} consecutive incorrect login attempts.",
            count, 
        );
        ...
        let msg: String = format!(
            "ERROR: The Client ID with the chosen KEM variant used an incorrect password. Attempts used {}/{}.",
            count, login_threshold, 
        );
        let _ = socket.write_all(msg.as_bytes()).await;
    }    
}