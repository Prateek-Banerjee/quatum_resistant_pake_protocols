...
use qr_pake_protocols::{AvailableVariants, KemChoice, OcakeClient};
...
pub async fn ocake_pake_register_client(
    client_id: &[u8],
    client_password: &[u8],
...
pub async fn ocake_pake_client_login(mut ocake_client: OcakeClient) -> Result<(f32, usize), Error> {
    ...
    while client_steps != ClientSteps::Done {
        match client_steps {
            ClientSteps::ClientInit => {
                ...
                // Send the message to the server
                stream.write_all(&client_message).await.unwrap();
                client_communication_cost += c.len() + nonce_c.len();
            ...
            ClientSteps::ClientFinish => {
                ...
                let (tag_two, _) = ocake_client
                    .client_finish(&kem_ciphertext, &tag_one)
                    .unwrap();

                total_client_time += client_timer.elapsed().as_secs_f32();

                // Serialize the data so that it can be sent over the network
                let serialized_data_to_be_sent: Vec<u8> = to_vec(&(tag_two.to_vec())).unwrap();
                ...
                // Send the message to the server
                stream.write_all(&client_message).await.unwrap();
                client_communication_cost += tag_two.len();
